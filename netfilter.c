...
bool netfilter(unsigned char* buf, char * Host){
    ...
    int i,j=0;
    int Hsize = strlen(Host);
    char text[Hsize];
    printf("tcplend + %d\n",ip_tlen-(ip_hlen+tcplen)*4);

    if (ntohs(tcp->th_dport) == 80) {  // 80 port
          ...
          struct nfq_data *nfa, char *Host)
{
    ...
    if (ret >= 0) {
        if (netfilter(data,Host)==true) {
            ...
            return nfq_set_verdict(qh, id, NF_DROP, 0, NULL);  //DROP or ACCEPT
        }
        else {
            printf("ACCEPT the Host\n");
            return nfq_set_verdict(qh, id, NF_ACCEPT, 0, NULL);  //DROP or ACCEPT
        }
    }
    printf("\n");
    return nfq_set_verdict(qh, id, NF_ACCEPT, 0, NULL);  //DROP or ACCEPT
